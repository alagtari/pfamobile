...
@RoutePage()
class UpdateTruckScreen extends StatefulWidget implements AutoRouteWrapper {
  const UpdateTruckScreen({super.key, required this.truck});
  final TruckModel truck;

  @override
  _UpdateTruckScreenState createState() => _UpdateTruckScreenState();
...
class _UpdateTruckScreenState extends State<UpdateTruckScreen> {
  ...
  void initState() {
    ...
    _numberController = TextEditingController(text: widget.truck.number);
    _modelController = TextEditingController(text: widget.truck.model);
    _capacityController =
        TextEditingController(text: widget.truck.capacity.toString());
  }
  ...
  void _onSubmit() {
    ...
    final truck = widget.truck.copyWith(
    ...
    context
        .read<TruckBloc>()
        .add(UpdateTruckEvent(truck: truck, id: truck.id!));
  }
      ...
      body: SafeArea(
        child: BlocListener<TruckBloc, TruckState>(
          listener: (context, state) {
            if (state is UpdateTruckSuccess) {
              showTopSnackBar(
                Overlay.of(context),
